  ...
  <Layout>
    ...
    <section className="py-20 bg-secondary/40">
      <div className="container grid md:grid-cols-2 gap-7">
        {causes.map((c, idx) => (
          <Reveal key={c.slug} delay={(idx % 2) * 120} as="article" className="group bg-card rounded-[28px] overflow-hidden shadow-card hover:shadow-elevated transition-all border border-border p-3">
            <Link to={`/causes/${c.slug}`} className="block">
