  ...
  <Layout>
    ...
    <section className="py-20 bg-secondary/40">
      <div className="container space-y-6">
        {causes.map((c, idx) => (
          <Reveal key={c.slug} delay={idx * 80} as="article" className="bg-card rounded-3xl overflow-hidden shadow-card hover:shadow-elevated transition-all border border-border grid md:grid-cols-[420px_1fr]">
            <Link to={`/programs/${c.slug}`} className="aspect-[4/3] md:aspect-auto overflow-hidden block">
            ...
            <div className="p-8 md:p-10 flex flex-col justify-center">
              ...
              <h3 className="font-bold text-2xl md:text-3xl leading-tight">
                <Link to={`/programs/${c.slug}`} className="hover:text-primary transition-colors">{c.title}</Link>
              </h3>
              ...
              <Button asChild className="mt-7 self-start rounded-full bg-foreground text-background hover:bg-primary hover:text-primary-foreground">
                <Link to={`/programs/${c.slug}`}>Learn More <ArrowRight className="ml-1 h-4 w-4" /></Link>
              </Button>
            </div>
          </Reveal>
        ))}
